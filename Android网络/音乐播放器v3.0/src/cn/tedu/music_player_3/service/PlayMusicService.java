...
public class PlayMusicService extends Service implements Consts {
	...
	private class InnerReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (ACTION_PLAY_OR_PAUSE.equals(action)) {
				...
				play(position);
			} else if (ACTION_PLAY_FROM_PERCENT.equals(action)) {
				// 从指定的百分比位置开始播放
		...
		public void run() {
			...
			while (isRunning) {
				if (player.isPlaying()) {
					...
					intent.putExtra(EXTRA_PERCENT, player.getDuration() == 0 ? 0 : player.getCurrentPosition() * 100 / player.getDuration());
	...
	private void play(float percent) {
		pausePosition = (int) (player.getDuration() * percent);
		play();
	}
