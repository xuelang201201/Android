...
public class MyTabIcon extends View {
	...
	Bitmap icon; // 因为只能绘制Bitmap类型的图片在屏幕上，所以需要将Drawable --> Bitmap
	...
	private void initPaint() {
		...
		// 画笔的颜色因为在绘制的过程中要反复修改，所以没必要在设置这里设置
	...
	private void initView(Context context, AttributeSet attrs) {
		...
		textColor = t.getColor(R.styleable.MyTabIcon_icon_color, Color.parseColor("#ff33b5e5"));
		// 设置字体大小
		int defValue = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_SP, 10, getResources().getDisplayMetrics());
		textSize = t.getDimensionPixelSize(R.styleable.MyTabIcon_icon_textsize, defValue);
	...
	protected void onDraw(Canvas canvas) {
		...
		// 图像的纵坐标（控件高度/2 - 图像高度/2）
		float top = getHeight() / 2 - icon.getHeight() / 2 - 20;
		canvas.drawBitmap(icon, left, top, drawPaint);
		// 画文字
		// 测量文字大小
		Rect bounds = new Rect();
		textPaint.getTextBounds(textContent, 0, textContent.length(), bounds);
		// (x, y)代表文字的左下角坐标
		// 文字的横坐标（控件宽度/2 - 文字宽度/2）
		float x = getWidth() / 2 - bounds.width() / 2;
		// 文字的纵坐标（控件高度/2 + 图片高度/2 + 文字高度）
		float y = getHeight() / 2 + icon.getHeight() / 2 + bounds.height() - 10;
		// 指定文字颜色
	...
	private void drawColorIcon(Canvas canvas, float left, float top) {
		...
		// 把刚才画带颜色图片时为画笔设置的颜色，透明度和混合模式统统去掉，恢复到最初时的画笔
		drawPaint.reset();
		// 把画好的bitmap画到屏幕上
		canvas.drawBitmap(bitmap, left, top, null); // drawPaint和null都行
	}
	...
	public void setMyTabIconAlpha(int alpha) {
		this.alpha = alpha;
		// 通过invalidate方法，让系统不断的去调用onDraw方法
		invalidate();
	}
}